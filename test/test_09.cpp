...
int main(int argc, char **argv)
{
    ...
    // initialize
    init_07 test_09 ("test_09", false);
    //-----------------------------------------------------------


    //-----------------------------------------------------------
    smpc::solver_as solver(
            test_09.wmg->N, // size of the preview window
            400.0,  // Alpha
            4000.0,  // Beta
            1.0,    // Gamma
            0.01,   // regularization
    ...
    //-----------------------------------------------------------
    test_09.par->init_state.set (0.019978839010709938, -6.490507362468014e-05);
    // state_tilde = state_orig, when velocity = acceleration = 0
    test_09.X_tilde.set (0.019978839010709938, -6.490507362468014e-05);
    //-----------------------------------------------------------


    FILE *file_op = fopen(test_09.fs_out_filename.c_str(), "a");
    ...
    test_09.wmg->T_ms[0] = control_sampling_time_ms;
    test_09.wmg->T_ms[1] = control_sampling_time_ms;

    for(int i=0 ;; i++)
    {
        cout << test_09.wmg->isSupportSwitchNeeded() << endl;
        if (test_09.wmg->formPreviewWindow(*test_09.par) == WMG_HALT)
        {
            cout << "EXIT (halt = 1)" << endl;
            break;
        }

        ZMP_ref_x.push_back(test_09.par->zref_x[0]);
        ZMP_ref_y.push_back(test_09.par->zref_y[0]);

        
        //------------------------------------------------------
        solver.set_parameters (test_09.par->T, test_09.par->h, test_09.par->h0, test_09.par->angle, test_09.par->zref_x, test_09.par->zref_y, test_09.par->lb, test_09.par->ub);
        solver.form_init_fp (test_09.par->fp_x, test_09.par->fp_y, test_09.par->init_state, test_09.par->X);
        solver.solve();
        //------------------------------------------------------
        // update state
        ...
        CoM_x.push_back(test_09.par->init_state.x());
        CoM_y.push_back(test_09.par->init_state.y());
    

        // feet position/orientation
        double left_foot_pos[16];
        double right_foot_pos[16];
        test_09.wmg->getFeetPositions (control_sampling_time_ms, left_foot_pos, right_foot_pos);
