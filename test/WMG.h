...
/** \brief Defines gains used in the Walking Motion Generator. */
class GainsWMG
{
 public:
  ...
  GainsWMG(double _Alpha, double _Beta, double _Gamma)  
    {
      set(_Alpha, _Beta, _Gamma);
    }
...
class WMG
{
 public:
   ...
   */
  WMG(int _N, double _T, double _hCoM, double Alpha = 150.0, double Beta = 2000.0, double Gamma = 1.0)
    {
  ...
  /** \brief Gains used to form the objective function(s).*/
  GainsWMG Gains;
  ...
  void form_FP_init(int output_flag = 0)
  {
    ...
    */
    ...
    if (output_flag)
      {
        if (counter == 0)
          write_file(FP_init, 8*N, 1, "output/FP.dat", "w"); 
        else
          write_file(FP_init, 8*N, 1, "output/FP.dat", "a"); 
      }
  }
